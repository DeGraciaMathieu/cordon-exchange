...
|---|---|
| État complet | `createApp({rng})` — `src/app.js`. Champs : `day`, `money`, `debt`, `rep{fac}`, `price{id}`, `history{id: prix[]}`, `inv{id}`, `exps[]`, `factionEvents[]`, `deliveryOffers[]`, `activeDeliveries[]`, `contractSeq`, `sellFaction`, `milestones{}`, `over`, `encounter`, `marketTip`, plus `rng` et `bus` |
