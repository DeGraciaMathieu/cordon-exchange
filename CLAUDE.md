...
7. **`nextDay(state)` (`src/app.js`) est la seule porte d'entrée de la simulation** ; les actions joueur (`buy`, `sell`, `haggle`, `launchExp`, `acceptDelivery`, `deliverContract`, `payRansom`, `selectFaction`, `chooseEncounter`, `recruitStalker`, `selectStalker`) sont les seules autres mutations.
